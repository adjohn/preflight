...
export const NR_HOOK_RE =
  /preflight-collector"?\s+(?:pre-tool|post-tool|stop-failure|session-start)/;
...
export function generateHookEntries(
  ...
  if (options?.platform === 'wsl-windows-cc') {
    ...
    sessionStart = `wsl.exe -e "${quotedPath}" session-start`;
  } else {
  ...
  return {
    ...
    SessionStart: [{ matcher: HOOK_MATCHER, hooks: [{ type: 'command', command: sessionStart }] }],
  };
}
 ...
 * Pure — no file I/O.
 *
 * Deliberately does NOT check StopFailure or SessionStart: broadening what
...
export function mergeSettings(
  ...
  for (const hookType of ['PreToolUse', 'PostToolUse', 'StopFailure', 'SessionStart'] as const) {
