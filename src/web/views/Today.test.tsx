...
describe('Today view', () => {
  ...
  it('shows the muted watcher-off line when watcherActive is false and no subagent turns have been recorded', async () => {
    ...
    renderToday();
    expect(await screen.findByText(/Subagent spend includes other sessions/)).toBeInTheDocument();
  });

  it('does not show the muted watcher-off line when the cross-session aggregate reports nonzero subagent turns, even though the live SSE turn count is 0', async () => {
    ...
    // one the "subagent spend" KPI already falls back to — reports turns for
    // today. The gate must agree with that KPI.
    globalThis.fetch = vi.fn(async (input) => {
      const url = String(input);
      if (url.includes('/api/observability-health')) {
        return new Response(
    ...
    expect(screen.queryByText(/Subagent spend includes other sessions/)).toBeNull();
  });
