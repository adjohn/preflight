...
- `curl` exit 7 with `Can't assign requested address`, or Chromium `net::ERR_ADDRESS_INVALID`, while doctor shows the process alive and owning the port, means the host has run out of ephemeral ports. It is not a Preflight fault. Check `netstat -an -p tcp | grep -c TIME_WAIT`, and wait or free sockets before calling a proof failed.
