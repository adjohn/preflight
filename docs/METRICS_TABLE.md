...
| `ai.bash.call_count`               | count      | `{category, session_id?, team_id?, project_id?, org_id?, repo_url?}` | Per-`bashCategory` call count for Bash tool calls (e.g. `git`, `test-runner`, `build`). Source: `SessionTracker.emitMetrics()` |
