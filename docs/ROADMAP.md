...
**Scope:**
- `normalizeDeveloperName()` in `packages/nr-ai-mcp-server/src/config.ts` — lowercase, collapse non-alphanumeric (except `-`) to `_`, strip leading/trailing `_`, truncate to 64 chars, fall back to `'unknown'`
